...
use crate::verify::{ObligationKind, VerifyErrorKind, analyze_obligations, verify_module};
...
fn old_in_precondition_rejected() {
    ...
    let errors =
        verify("module M entity X { v: Int } action A { x: X requires { x.v > old(x.v) } }");
...
fn postcondition_without_params() {
    let errors =
        verify("module M entity X { v: Int } action A { ensures { forall x: X => x.v > 0 } }");
    assert!(
        errors.contains(&VerifyErrorKind::PostconditionWithoutParams {
            function: "A".into(),
        })
    );
}
...
fn unknown_quantifier_type() {
    let errors = verify("module M action A { x: Int requires { forall y: Unknown => y > 0 } }");
...
fn quantifier_over_action_accepted() {
    ...
    let has_unknown = errors
        .iter()
        .any(|e| matches!(e, VerifyErrorKind::UnknownQuantifierType { .. }));
    assert!(
        !has_unknown,
        "Transfer should be accepted as quantifier type, got: {:?}",
        errors
    );
}
...
fn union_variant_not_flagged_as_unbound() {
    ...
    let has_unbound = errors
        .iter()
        .any(|e| matches!(e, VerifyErrorKind::UnboundVariable { .. }));
    assert!(
        !has_unbound,
        "union variants should not be flagged, got: {:?}",
        errors
    );
}
...
fn old_in_temporal_invariant_accepted() {
    ...
    let has_old_err = errors
        .iter()
        .any(|e| matches!(e, VerifyErrorKind::OldOutsidePoscondition));
    assert!(
        !has_old_err,
        "old() should be valid in temporal invariants, got: {:?}",
        errors
    );
}
...
fn zero_arg_call_not_flagged() {
    let errors =
        verify("module M action A { x: Int requires { x > now() } ensures { x == now() } }");
    let has_unbound = errors
        .iter()
        .any(|e| matches!(e, VerifyErrorKind::UnboundVariable { name } if name == "now"));
    assert!(
        !has_unbound,
        "now() should not be flagged as unbound, got: {:?}",
        errors
    );
}
...
fn transfer_example_verifies() {
    ...
    assert!(
        errors.is_empty(),
        "transfer.intent should verify cleanly, got: {:?}",
        errors
    );
}
...
fn auth_example_verifies() {
    ...
    assert!(
        errors.is_empty(),
        "auth.intent should verify cleanly, got: {:?}",
        errors
    );
}
...
fn shopping_cart_example_verifies() {
    ...
    assert!(
        errors.is_empty(),
        "shopping_cart.intent should verify cleanly, got: {:?}",
        errors
    );
}
...
fn transfer_example_obligations() {
    ...
    assert_eq!(obs.len(), 2);
    let preservation = obs
        .iter()
        .find(|o| o.kind == ObligationKind::InvariantPreservation);
    let temporal = obs
        .iter()
        .find(|o| o.kind == ObligationKind::TemporalProperty);
    assert!(
        preservation.is_some(),
        "expected InvariantPreservation obligation"
    );
